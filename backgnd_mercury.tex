...
\peter{Better to make this look like a grammar:  $\mathit{decl} \to
  \mathit{name}(\mathit{var},\ldots,\mathit{var}) \texttt{:-}
  \mathit{goal}$ etc.
It's best to put terminal is typewriter font, and nonterminals in math
italic.}
...
\peter{Why give pred declaration, but not the type declaration for
  list or the mode declarations for append?  Maybe just leave out the
  pred declaration.}
...
A variable may be free, ground, clobbered or bound to any of a set of
function symbols.
\peter{Probably best to leave out partial instantiation altogether.}
...
\peter{Wrong quotes here:  use backquote for left single quote, and
  two backquotes for left double quote.  Do a query replace throughout
  your thesis.}
...
:- mode uo == free >> unique.
\end{verbatim}
\peter{Looks like you'll need to discuss the \verb+>>+ operator and
  the whole inst system.  Or handwave through it here, and leave out
  the Mercury definitions of these modes.  The latter is probably good
  enough.}
...
of calls.
\peter{I think this is simpler if you leave out partial instantiation,
  and stick to ground and free.}
This means that,
...
\peter{also mention the else port}
...
\peter{``used''?  Unless explicitly quantified, all variables are
  visible throughout the clause.}
 outside the branched goal,
  with one exception:
...
\peter{Can we just leave out the \verb+cc_*+ determinisms?  They don't
  really add anything to your work, and handling them seems pretty
  straightforward.}
...
The \I/O module also defines a large set of predicates that perform I/O.
...
\peter{Unclear how you ``wrap'' a word around a goal.  Don't you
  really \textit{precede} the goal with the promise?}
