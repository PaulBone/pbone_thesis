...
Mercury has a strong Hindley-Milner~\cite{hindley_milner} type system very
similar to Haskell's~\cite{haskell}.
...
A variable may be free, ground or guaranteed to be bound to any of a set of
function symbols.
...
The mode of a predicate's argument is a transition between two instantiation states,
arguments can only become 'more instantiated',
that is to say 'free to ground' is legal but 'ground to free' is not.
Similarly uniqueness cannot be added to a value that is at least bound.
...
the caller will not be able to use it after the call therefore it must be unique.
A variable passed in a unique output argument will be instantiated and
