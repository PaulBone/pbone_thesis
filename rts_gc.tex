...
Likewise, a call can not modify global memory.
...
Therefore,
during memory allocation a thread uses its own free lists rather than
...
$|N|$ is always smaller than 2 where $N_{i+1} = N_{i}^2 + C$ and $N_0 = 0$.
...
\label{cabsav}
We ran our benchmarks on 'cabsav',
...
whereby mandelbrot\_highalloc has better speedups because of parallelism in
...
This is deliberate because, by definition allocation can not occur during
collector time.
