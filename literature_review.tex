...
Variable bindings are also easy to manage in independent AND-parallelism,
...
Therefore, it si better to create fewer coarse-grained parallel tasks rather
...
at compile time which can be evaluated at runtime to estimate if the cost
of a goal is above a threshold for parallel execution given
...
Whilst the \citeauthor{king:lower_bound_time_complexity} paper
creates its cost functions from a convex hull approximation of the
program's costs. 
...
granularity control.
They measure the ``distance'' between the creation of successive parallel
tasks.
...
Mercury,
a pure logic/functional language, with strong and static type, mode and
determinism systems.
...
This allows \citeauthor{wang:2006:hons} to implement a transformation that efficiently
handles dependent AND-parallelism
...
parallelism as most semi-deterministic code has shorter runtimes.
...
is that unification an order of magnitude simpler in Mercury than in
...
this was only a syntactic sugar for a pure functional language.
...
as one iteration is being completed another iteration can be started.
It could also handle completely independent parallel loops.
Sisal's for loops included a reduction clause,
which states how the iterations' results are summarised into an overall
result,
which may be a scalar result, an array or stream.
...
parallel programs.
Information about Sisal is hard to find and access,
and so it is difficult to say anything about how well the system transformed
programs into executable code and how well it performed.
...
NESL calls these structures sequences, and
...
parallelism and cannot be parallelised.
...
As well as other things that can affect parallel execution performance
including
...
Therefore we advocate the use of feedback directed parallelism;
this technique gathers cost information from a previous execution of the
program,
and uses that information to estimate the costs of the program's
computations in the future, parallelising it accordingly.
...
then it has the same problems with lazyness that the introduction of
...
by the other usually do not correlate with the number of dependant variables.
Tannier's algorithm was, in general, too optimistic
about the parallelism available in dependant conjunctions.
Tannier also made use of compile-time granularity control to reduce the
over-parallelisation that can occur in recursive code.

To improve on Tannier's methods my honours project
