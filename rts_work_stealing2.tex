...
\status{
    A few things to fix up before this section is ready, but it's mostly
    done now.
}
...
Fibonacci numbers where these are defined recursivly.
...
The naive fibs program is an excelent micro-benchmark for work stealing.
...
To avoid embarassing parallelism and create a point for comparison,
...
We can acheive this by introducing a new function \fibsgc,
...
In parallelised divide and conqure code such as Fibs the context limit does
not affect performace as badly as it does with right
...
because in the divide and conqure case both conjuncts have roughly
equivilent costs.
...
they are also faster than those without granulaity control.
...
the sparks on its own deque except one, which it executes immediatly.
...
We have considered useing the deque described by
