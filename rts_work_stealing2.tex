...
\begin{description}
    ...
    would not exceed the contest limit by stealing and executing the spark.
...
In left recursive mandelbrot all there versions are comparable.
The old implementation used one spark deque per context, of which there are
many in right-recursive code;
where as the new implementation uses one spark deque per engine, of which
...
This is not a big surprise as the cost of notification can be occurred any
time a spark is created,
the cost can vary depending if a notification needs to be made or not.
