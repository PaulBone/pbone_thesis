...
Engines can notify one another,
...
An engine that creates a spark or makes a context runnable can
send a message to another engine.
The sender writes to the \code{action} and \code{action\_data} fields.
...
Such a sender will use a compare and swap operation to write \code{MR\_BUSY}
into the state field before updating the \code{action} and
\code{action\_data} fields.
