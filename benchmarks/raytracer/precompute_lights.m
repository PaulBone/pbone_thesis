...
:- type bounding_sphere
    ...
    % Take a composite scene, return a list of the bounding boxes
    % for objects inside it.
    %
:- pred scene_list(scene::in, list(bounding_sphere)::out) is det.

    % pre_compute_lighting calculates the "cleanly illuminated" (i.e.
    % no chance of any shadows) light list for every basic object in the scene.
    %
...
:- pred calc_light_list(object::in(basic_object_inst),
	maybe(transformation)::in, list(bounding_sphere)::in,
	list(light)::in, object::out) is det.

calc_light_list(Obj, MaybeTrans, BSList, Lights, NewObject) :-
    % planes *might* go in here, if all objects are on the opposite side
    % of them... but it's probably not worth the effort.
...
is_clear(Centre, Radius, BSList, Light) :-
    ...
    %
:- pred single_is_clear(point::in, real::in, light::in, bounding_sphere::in)
    is semidet.
...
%------------------------------------------------------------------------------%
    ...
    % bounding_sphere takes a pair of points and returns their
    % "centre" and "radius".
    %
