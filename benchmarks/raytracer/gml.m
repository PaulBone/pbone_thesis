...
conv_string_to_int(String, Base, Token, !IO) :-
    ...
    ;
        ...
        Msg = string.append_list(["invalid int token `", String, "'"]),
...
conv_to_float(String, Token, !IO) :-
    ...
    ;
        io.get_line_number(Line, !IO),
        Msg = string.append_list(["invalid float token `", String, "'"]),
        throw(lexer_error(Line, Msg))
    ).
...
lexer_unexpected_char(Char, Where, !IO) :-
    ...
    Msg = string.append_list(
        ["unexpected character `",
            string.from_char_list([Char]), "' in ", Where]),
    throw(lexer_error(Line, Msg)).
...
parse_2(StopAt, [Token | Tokens], RemainingTokens, Prog0, Prog) :-
    ...
    ;
        ...
        parse_2(StopAt, Tokens1, RemainingTokens, [array(Array) | Prog0], Prog)
    ;
        Token = ']',
        ( StopAt = end_array ->
