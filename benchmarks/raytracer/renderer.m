...
:- pred set_dependent(int::in, io::di, io::uo) is det.
...
render(Params1, !IO) :-
    ...
    (
        ...
        io.set_binary_output_stream(BinStream, _Old, !IO),
        ...
        get_dependent(Dependent, !IO),
        get_target_parallelism(TargetPar, !IO),
        ( Dependent = 0 ->
            ( TargetPar = 0 ->
                Task = render_all_rows_indep(Params2)
            ;
                ChunkSize = (Height + TargetPar - 1) / TargetPar,
                Task = render_chunk_rows_indep(Params2, ChunkSize)
            )
        ;
            ( TargetPar = 0 ->
                Task = render_all_rows_dep(Params2)
            ;
                ChunkSize = (Height + TargetPar - 1) / TargetPar,
                Task = render_chunk_rows_dep(Params2, ChunkSize)
            )
        ),

        % Render the image.
        gettimeofday(T0, !IO),
        Task(0, Width, Height, cord.empty, Bytes),
        gettimeofday(T1, !IO),
        ...
        % Write out the image.
        ByteList = cord.list(Bytes),
        list.foldl(
            ( pred(Byte::in, IO0::di, IO::uo) is det :-
...
%---------------------------------------------------------------------------%
    ...
    % Print rows [StartRow .. Height).
    %
:- pred render_chunk_rows_dep(render_params::in, int::in, int::in,
    int::in, int::in, cord(int)::in, cord(int)::out) is det.

render_chunk_rows_dep(Params, ChunkSize, StartRow, Width, Height, !Bytes) :-
    ( StartRow >= Height ->
        true
    ;
        EndRow0 = StartRow + ChunkSize - 1,
        ( EndRow0 =< Height ->
            EndRow = EndRow0
        ;
            EndRow = Height
        ),
        trace [compiletime(flag("DEBUG")), io(!IO)] (
            io.format(io.stderr_stream, "dep rows %d - %d\n",
                [i(StartRow), i(EndRow)], !IO),
            io.flush_output(io.stderr_stream, !IO)
        ),
        (
            render_some_rows(Params, StartRow, EndRow, Width,
                cord.empty, RowBytes),
            !:Bytes = !.Bytes ++ RowBytes
        &
            render_chunk_rows_dep(Params, ChunkSize, EndRow + 1,
                Width, Height, !Bytes)
        )
    ).
    ...
    % Print rows [StartRow .. Height).
    %
:- pred render_chunk_rows_indep(render_params::in, int::in, int::in,
    int::in, int::in, cord(int)::in, cord(int)::out) is det.

render_chunk_rows_indep(Params, ChunkSize, StartRow, Width, Height, !Bytes) :-
    ( StartRow >= Height ->
        true
    ;
        EndRow0 = StartRow + ChunkSize - 1,
        ...
        trace [compiletime(flag("DEBUG")), io(!IO)] (
            io.format(io.stderr_stream, "indep rows %d - %d\n",
                [i(StartRow), i(EndRow)], !IO),
            io.flush_output(io.stderr_stream, !IO)
        ),
        (
            render_some_rows(Params, StartRow, EndRow, Width,
                cord.empty, RowBytes)
        &
            render_chunk_rows_indep(Params, ChunkSize, EndRow + 1,
                Width, Height, cord.empty, RowsBytes)
        ),
    ...
    %
:- pred render_some_rows(render_params::in, int::in, int::in, int::in,
    cord(int)::in, cord(int)::out) is det.

render_some_rows(Params, StartRow, EndRow, Width, !Bytes) :-
    ( StartRow > EndRow ->
        true
    ;
        render_one_row(Params, StartRow, Width, cord.empty, RowBytes),
        !:Bytes = !.Bytes ++ RowBytes,
        render_some_rows(Params, StartRow + 1, EndRow, Width, !Bytes)
    ).
...
%---------------------------------------------------------------------------%
    ...
    % Print rows [StartRow .. Height).
    %
:- pred render_all_rows_dep(render_params::in, int::in, int::in, int::in,
    cord(int)::in, cord(int)::out) is det.

render_all_rows_dep(Params, StartRow, Width, Height, !Bytes) :-
    ( StartRow >= Height ->
        true
    ;
        trace [compiletime(flag("DEBUG")), io(!IO)] (
            io.format(io.stderr_stream, "dep row %d\n", [i(StartRow)], !IO),
            io.flush_output(io.stderr_stream, !IO)
        ),
        (
            render_one_row(Params, StartRow, Width, cord.empty, RowBytes),
            !:Bytes = !.Bytes ++ RowBytes
        &
            render_all_rows_dep(Params, StartRow + 1, Width, Height, !Bytes)
        )
    ).

    % Print rows [StartRow .. Height).
    %
:- pred render_all_rows_indep(render_params::in, int::in, int::in, int::in,
    cord(int)::in, cord(int)::out) is det.

render_all_rows_indep(Params, StartRow, Width, Height, !Bytes) :-
    ( StartRow >= Height ->
        true
    ;
        trace [compiletime(flag("DEBUG")), io(!IO)] (
            io.format(io.stderr_stream, "indep row %d\n", [i(StartRow)], !IO),
            io.flush_output(io.stderr_stream, !IO)
        ),
        (
            render_one_row(Params, StartRow, Width, cord.empty, RowBytes)
        &
            render_all_rows_indep(Params, StartRow + 1, Width, Height,
                cord.empty, RowsBytes)
        ),
        !:Bytes = !.Bytes ++ RowBytes ++ RowsBytes
    ).
...
:- pred render_one_row(render_params::in, int::in, int::in,
    cord(int)::in, cord(int)::out) is det.

render_one_row(Params, Row, Width, !Bytes) :-
    trace [compiletime(flag("ROWS")), io(!IO)] (
        io.format(io.stderr_stream, "generating row %d\n", [i(Row)], !IO),
        io.flush_output(io.stderr_stream, !IO)
    ),
    render_columns(Params, 0, Row, Width, !Bytes).

    % Print colimns [StartCol .. Width).
    %
:- pred render_columns(render_params::in, int::in, int::in, int::in,
    cord(int)::in, cord(int)::out) is det.

render_columns(Params, StartCol, Row, Width, !Bytes) :-
    ( StartCol = Width ->
        true
    ;
        ...
        render_columns(Params, StartCol + 1, Row, Width, !Bytes)
    ).
...
:- pred choose_closest_intersection(position::in, vector::in,
    intersection_result::in, object_id::in, maybe(best_intersection)::out)
    is det.
...
find_object_intersection(union(Object1, Object2), MaybeTransformation,
    ...
    find_surface_and_not_object_intersection(Object1, Object2,
        MaybeTransformation, RayOrigin, RayDirection, Intersection1),
...
:- pred find_surface_and_object_intersection(object::in, object::in,
    maybe(transformation)::in, point::in, vector::in, intersection_result::out)
    is det.
...
compute_intensity(RenderParams, RayDirection, IgnoreId, IntersectionPoint,
    ...
    %
:- pred position_not_in_shadow(render_params::in, object_id::in,
    position::in, value::in) is semidet.
...
light_is_behind_point(Vantage, directional(Dir, _), Point) :-
    % Ensure that the Point is in the same direction as the light source.
...
eye_position = point(0.0, 0.0, -1.0).
    ...
    % given the fov (in degrees), return the image width in world space.
    %
:- func image_width(degrees) = real.

image_width(Fov) = 2.0 * image_halfwidth(Fov).

    % Given the fov (in degrees), return half the image width in world space.
    %
...
"
    MR_Integer   dependent = 0;
    MR_Integer   target_parallelism = 0;
    MR_String    time_filename = (MR_String) (MR_Integer) """";
").

:- pragma foreign_proc("C",
    set_dependent(Dependent::in, IO0::di, IO::uo),
    [will_not_call_mercury, promise_pure, thread_safe],
"
    extern MR_Integer   dependent;

    dependent = Dependent;
    IO = IO0;
").

:- pred get_dependent(int::out, io::di, io::uo) is det.

:- pragma foreign_proc("C",
    get_dependent(Dependent::out, IO0::di, IO::uo),
    [will_not_call_mercury, promise_pure, thread_safe],
"
    extern MR_Integer   dependent;

    Dependent = dependent;
    /* fprintf(stderr, ""dependent parallelism %ld\\n"", (long) dependent); */
    IO = IO0;
").
