...
:- pred eval_error(env::in, stack::in) is erroneous.
    ...
    % maybe holds the number of results the operator puts back onto the stack.
    %
...
:- pred state_add_render_command(render_params::in, state::in, state::out)  
    is det.
...
state_add_render_command(Params, !State) :-
    some [!RenderCommands] (
        ...
        !State ^ s_render_commands := !.RenderCommands
    ).
...
do_token_group(single_token(Token), Env0, Stack0, Env, Stack, !State) :-
    do_token(Token, Env0, Stack0, Env, Stack, !State).
do_token_group(function(TokenList), Env0, Stack0, Env, Stack, !State) :-
    ...
    Env = Env0.
do_token_group(array(TokenList), Env0, Stack0, Env, Stack, !State) :-
    interpret(TokenList, Env0, empty_stack, _ResultEnv, ArrayStack, !State),
    Stack = push(array(array(reverse(ArrayStack))), Stack0),
    Env = Env0.

:- pred do_token(token::in, env::in, stack::in, env::out, stack::out,
    state::in, state::out) is det.

do_token(operator(Operator), Env0, Stack0, Env, Stack, !State) :-
    Env = Env0,
    do_op(Operator, Env, Stack0, Stack, !State).
do_token(identifier(Id), Env0, Stack0, Env, Stack, !State) :-
    (
        map.search(Env0, Id, Val)
    ->
        Stack = push(Val, Stack0),
        Env = Env0
    ;
        throw(program_error(string.append_list(
            ["identifier `", Id, "' is unknown"])))
    ).
do_token(binder(Id), Env0, Stack0, Env, Stack, !State) :-
    ( pop(Stack0, Val, Stack1) ->
        Stack = Stack1,
        map.set(Id, Val, Env0, Env)
...
do_token(boolean(Bool), Env0, Stack0, Env, Stack, !State) :-
    Stack = push(boolean(Bool), Stack0),
    Env = Env0.
do_token(number(integer(Int)), Env0, Stack0, Env, Stack, !State) :-
    Stack = push(int(Int), Stack0),
    Env = Env0.
do_token(number(real(Real)), Env0, Stack0, Env, Stack, !State) :-
    Stack = push(real(Real), Stack0),
    Env = Env0.
do_token(string(String), Env0, Stack0, Env, Stack, !State) :-
    Stack = push(string(String), Stack0),
    Env = Env0.
do_token(extra(Operator), Env0, Stack0, Env, Stack, !State) :-
    Env = Env0,
    do_extra(Operator, Env, Stack0, Stack, !State).
...
do_op(cos, Env, Stack0, Stack, !State) :-
    ( Stack0 = [real(N) | Stack1] ->
        Stack = push(real(op_cos(N)), Stack1)
    ;
        eval_error(Env, Stack0)
    ).
do_op(cube, Env, Stack0, Stack, !State) :- %Tier-2
    ( Stack0 = [closure(CEnv, CCode) | Stack1] ->
        next_object_id(Id, !State),
        Stack = push(object(
...
do_op(cylinder, Env, Stack0, Stack, !State) :- %Tier-2
    ( Stack0 = [closure(CEnv, CCode) | Stack1] ->
        next_object_id(Id, !State),
        Stack = push(object(
...
do_op(if, Env, Stack0, Stack, !State) :-
    ...
    ->
        (
            YesNo = yes,
            interpret(CC1, CE1, Stack1, _ResultEnv, Stack, !State)
        ;
            YesNo = no,
            interpret(CC2, CE2, Stack1, _ResultEnv, Stack, !State)
        )
    ;
        eval_error(Env, Stack0)
    ).
do_op(intersect, Env, Stack0, Stack, !State) :- %Tier-3
    ( Stack0 = [object(O2), object(O1) | Stack1] ->
        Stack = push(object(intersect(O1, O2)), Stack1)
    ;
        eval_error(Env, Stack0)
    ).
do_op(length, Env, Stack0, Stack, !State) :-
...
do_op(render, Env, Stack0, Stack, !State) :-
    ...
    ->
        ...
        Params = render_params(Amb, Lights, Scene, Depth, FOV, Wid, Ht, File),
        state_add_render_command(Params, !State),
        Stack = Stack1
    ;
        eval_error(Env, Stack0)
    ).
do_op(rotatex, Env, Stack0, Stack, !State) :-
    ( Stack0 = [real(Theta), object(Obj0) | Stack1] ->
        renameObject(Obj0, Obj, !State),
        Stack = push(object(transform(Obj, rotatex(Theta))), Stack1)
    ;
        eval_error(Env, Stack0)
    ).
do_op(rotatey, Env, Stack0, Stack, !State) :-
    ( Stack0 = [real(Theta), object(Obj0) | Stack1] ->
        renameObject(Obj0, Obj, !State),
...
do_op(rotatez, Env, Stack0, Stack, !State) :-
    ( Stack0 = [real(Theta), object(Obj0) | Stack1] ->
        renameObject(Obj0, Obj, !State),
...
do_op(scale, Env, Stack0, Stack, !State) :-
...
do_op(sin, Env, Stack0, Stack, !State) :-
    ( Stack0 = [real(N) | Stack1] ->
        Stack = push(real(op_sin(N)), Stack1)
    ;
        eval_error(Env, Stack0)
    ).
do_op(sphere, Env, Stack0, Stack, !State) :-
    ( Stack0 = [closure(CEnv, CCode) | Stack1] ->
        next_object_id(Id, !State),
        Stack = push(object(
...
do_op(spotlight, Env, Stack0, Stack, !State) :-
    (
...
do_op(union, Env, Stack0, Stack, !State) :-
    ( Stack0 = [object(O2), object(O1) | Stack1] ->
        Stack = push(object(union(O1, O2)), Stack1)
    ;
        eval_error(Env, Stack0)
    ).
do_op(uscale, Env, Stack0, Stack, !State) :-
...
%-----------------------------------------------------------------------------%
    ...
    %
:- pred renameObject(object::in, object::out, state::in, state::out) is det.

renameObject(basic_object(_, BasicObject,L), basic_object(Id,BasicObject,L),
        !State) :-
    next_object_id(Id, !State).
renameObject(transform(Obj0, Trans), transform(Obj, Trans), !State) :-
    renameObject(Obj0, Obj, !State).
renameObject(union(Left0, Right0), union(Left, Right), !State) :-
    renameObject(Left0, Left, !State),
    renameObject(Right0, Right, !State).
renameObject(intersect(Left0, Right0), intersect(Left, Right), !State) :-
    renameObject(Left0, Left, !State),
    renameObject(Right0, Right, !State).
renameObject(difference(Left0, Right0), difference(Left, Right), !State) :-
    renameObject(Left0, Left, !State),
    renameObject(Right0, Right, !State).
...
next_object_id(Id, !State) :-
    ...
    !State ^ s_global_object_counter := Id + 1.
...
do_extra(Extra0, Env, Stack0, Stack, !State) :-
    extra_operator_mode(Extra0, Extra),
    do_extra2(Extra, Env, Stack0, Stack, !State).

:- pragma inline(do_extra2/6).

:- pred do_extra2(extra_operator::in(extra_operator_inst), env::in,
    stack::in, stack::out, state::in, state::out) is det.

do_extra2(mercury_closure(C), Env, Stack0, Stack, !State) :-
    C(Env, Stack0, _, Stack, !State).
do_extra2(dup, Env, Stack0, Stack, !State) :-
    ( Stack0 = [Head | Tail] ->
        Stack = [Head, Head | Tail]
    ;
        eval_error(Env, Stack0)
    ).
do_extra2(popn(N), Env, Stack0, Stack, !State) :-
    ( popn(N, Stack0, Stack1) ->
        Stack = Stack1
    ;
        eval_error(Env, Stack0)
    ).
do_extra2(constant_sphere(SurfaceProperties), _Env, Stack0, Stack, !State) :-
    next_object_id(Id, !State),
    Stack = push(object(
            basic_object(Id, sphere(constant(SurfaceProperties)), [])),
            Stack0).
do_extra2(constant_plane(SurfaceProperties), _Env, Stack0, Stack, !State) :-
    next_object_id(Id, !State),
    Stack = push(object(
            basic_object(Id, plane(constant(SurfaceProperties)), [])),
            Stack0).
do_extra2(constant_cone(SurfaceProperties), _Env, Stack0, Stack, !State) :-
    next_object_id(Id, !State),
    Stack = push(object(
            basic_object(Id, cone(constant(SurfaceProperties)), [])),
            Stack0).
do_extra2(constant_cube(SurfaceProperties), _Env, Stack0, Stack, !State) :-
    next_object_id(Id, !State),
    Stack = push(object(
            basic_object(Id, cube(constant(SurfaceProperties)), [])),
            Stack0).
do_extra2(constant_cylinder(SurfaceProperties), _Env, Stack0, Stack, !State) :-
    next_object_id(Id, !State),
    Stack = push(object(
            basic_object(Id, cylinder(constant(SurfaceProperties)), [])),
            Stack0).
do_extra2(constant_point(Point), _Env, Stack0, Stack, !State) :-
    Stack = push(point(Point), Stack0).
do_extra2(constant_if(C1, C2), Env, Stack0, Stack, !State) :-
    (
