...
peephole(Tokens0, list.reverse(Tokens)) :-
	peephole_2([], list.reverse(Tokens0), Tokens1,
			state(map.init), _),
	Tokens = Tokens1.
...
%	peephole_2([], Tokens1, Tokens, state(map.init), _).
	...
	%
:- pred peephole_2(list(string)::in, code::in, code::out,
    peephole::in, peephole::out) is det.

peephole_2(_, [], []) --> [].
peephole_2(UsedIds0, [Token | Tokens], NewTokens) -->
	{ Token = single_token(_) },
	{ UsedIds = used_ids([Token]) `list.append` UsedIds0 },
	peephole_2(UsedIds, Tokens, NewTokens0),
	( peephole.match(UsedIds0, Token, NewTokens0, Result) ->
		{ NewTokens = Result }
	;
...
peephole_2(UsedIds0, [Token | Tokens], NewTokens) -->
	...
	{ UsedIds = used_ids(TokenList) `list.append` UsedIds0 },
	peephole_2(UsedIds, Tokens, NewTokens0),

		% Process the function
	=(State),
	{ peephole_2(UsedIds0, list.reverse(TokenList), OptTokenList,
			State, _) },
...
:- pred peephole.insert(string::in, token_group::in,
    peephole::in, peephole::out) is det.

peephole.insert(Id, Token, State0, State) :-
	State = State0 ^ known_ids := map.set(State0 ^ known_ids, Id, Token).

:- pred peephole.delete(string::in, peephole::in, peephole::out) is det.

peephole.delete(Id, State0, State) :-
	State = State0 ^ known_ids := map.delete(State0 ^ known_ids, Id).

:- pred peephole.search(string::in, maybe(token_group)::out,
    peephole::in, peephole::out) is det.

peephole.search(Id, MaybeToken, State, State) :-
...
%------------------------------------------------------------------------------%
	...
	% Determine all the identifiers that are used in a piece of code.
    %
...
used_ids([function(TokenList) | Tokens])
	= used_ids(TokenList) ++ used_ids(Tokens).
...
%------------------------------------------------------------------------------%
	...
	%	- use dup to avoid multiple lookups or binds followed by lookups
	%	- avoid redundant lookups followed by binds or vice versa
	%	- introduce Mercury closures for some important patterns
	%
:- pred peephole.match(list(string)::in, token_group::in, token_list::in,
...
peephole.match(_UsedIds, single_token(operator(apply)), Args, Result) -->
	{ semidet_fail },
	{ Args = [function(FunctionTokens) | Rest] },
	{ Result = list.append(list.reverse(FunctionTokens), Rest) }.

	% Real/Integer constant propagation.
peephole.match(_UsedIds, single_token(operator(acos)), Args, Result) -->
	{ Args = [real_token(A) | Rest] },
	{ Result = [real_token(op_acos(A)) | Rest] }.

peephole.match(_UsedIds, single_token(operator(addi)), Args, Result) -->
	{ top_two_integer_args(Args, A, B, Rest) },
	{ Result = [integer_token(op_addi(A, B)) | Rest] }.

peephole.match(_UsedIds, single_token(operator(addf)), Args, Result) -->
	{ top_two_real_args(Args, A, B, Rest) },
	{ Result = [real_token(op_addf(A, B)) | Rest] }.

peephole.match(_UsedIds, single_token(operator(asin)), Args, Result) -->
	{ Args = [real_token(A) | Rest] },
	{ Result = [real_token(op_asin(A)) | Rest] }.

peephole.match(_UsedIds, single_token(operator(clampf)), Args, Result) -->
	{ Args = [real_token(A) | Rest] },
	{ Result = [real_token(op_clampf(A)) | Rest] }.

peephole.match(_UsedIds, single_token(operator(cos)), Args, Result) -->
	{ Args = [real_token(A) | Rest] },
	{ Result = [real_token(op_cos(A)) | Rest] }.

peephole.match(_UsedIds, single_token(operator(divi)), Args, Result) -->
	{ top_two_integer_args(Args, A, B, Rest) },
	{ Result = [integer_token(op_divi(A, B)) | Rest] }.

peephole.match(_UsedIds, single_token(operator(divf)), Args, Result) -->
	(
		{ top_two_real_args(Args, A, B, Rest) }
	->
...
peephole.match(_UsedIds, single_token(operator(eqi)), Args, Result) -->
	{ top_two_integer_args(Args, A, B, Rest) },
	{ Result = [boolean_token(op_eqi(A, B)) | Rest] }.

peephole.match(_UsedIds, single_token(operator(eqf)), Args, Result) -->
	{ top_two_real_args(Args, A, B, Rest) },
	{ Result = [boolean_token(op_eqf(A, B)) | Rest] }.

peephole.match(_UsedIds, single_token(operator(floor)), Args, Result) -->
	{ Args = [real_token(A) | Rest] },
	{ Result = [integer_token(op_floor(A)) | Rest] }.

peephole.match(_UsedIds, single_token(operator(frac)), Args, Result) -->
	{ Args = [real_token(A) | Rest] },
	{ Result = [real_token(op_frac(A)) | Rest] }.

peephole.match(_UsedIds, single_token(operator(lessi)), Args, Result) -->
	{ top_two_integer_args(Args, A, B, Rest) },
	{ Result = [boolean_token(op_lessi(A, B)) | Rest] }.

peephole.match(_UsedIds, single_token(operator(lessf)), Args, Result) -->
	{ top_two_real_args(Args, A, B, Rest) },
	{ Result = [boolean_token(op_lessf(A, B)) | Rest] }.

peephole.match(_UsedIds, single_token(operator(modi)), Args, Result) -->
	{ top_two_integer_args(Args, A, B, Rest) },
	{ Result = [integer_token(op_modi(A, B)) | Rest] }.

peephole.match(_UsedIds, single_token(operator(muli)), Args, Result) -->
	{ top_two_integer_args(Args, A, B, Rest) },
	{ Result = [integer_token(op_muli(A, B)) | Rest] }.

peephole.match(_UsedIds, single_token(operator(mulf)), Args, Result) -->
	{ top_two_real_args(Args, A, B, Rest) },
	{ Result = [real_token(op_mulf(A, B)) | Rest] }.

peephole.match(_UsedIds, single_token(operator(negi)), Args, Result) -->
	{ Args = [integer_token(A) | Rest] },
	{ Result = [integer_token(op_negi(A)) | Rest] }.

peephole.match(_UsedIds, single_token(operator(negf)), Args, Result) -->
	{ Args = [real_token(A) | Rest] },
	{ Result = [real_token(op_negf(A)) | Rest] }.

peephole.match(_UsedIds, single_token(operator(real)), Args, Result) -->
	{ Args = [integer_token(A) | Rest] },
	{ Result = [real_token(op_real(A)) | Rest] }.

peephole.match(_UsedIds, single_token(operator(sin)), Args, Result) -->
	{ Args = [real_token(A) | Rest] },
	{ Result = [real_token(op_sin(A)) | Rest] }.

peephole.match(_UsedIds, single_token(operator(sqrt)), Args, Result) -->
	{ Args = [real_token(A) | Rest] },
	{ A >= 0.0 },
	{ Result = [real_token(op_sqrt(A)) | Rest] }.

peephole.match(_UsedIds, single_token(operator(subi)), Args, Result) -->
	{ top_two_integer_args(Args, A, B, Rest) },
	{ Result = [integer_token(op_subi(A, B)) | Rest] }.

peephole.match(_UsedIds, single_token(operator(subf)), Args, Result) -->
	{ top_two_real_args(Args, A, B, Rest) },
	{ Result = [real_token(op_subf(A, B)) | Rest] }.

peephole.match(_UsedIds, single_token(operator(if)), Args, Result) -->
	{
		% Branch reduction
	...
	->
		(
			YesNo = yes,
			Result = list.reverse(True)
				`list.append` Rest
		;
			YesNo = no,
			Result = list.reverse(False)
				`list.append` Rest
		)
	;
	...
	% instruction.
peephole.match(UsedIds, single_token(binder(Id)), Args, Result) -->
	...
	;
		...
		;
			...
			( peephole.match(UsedIds, NewToken, Args, Result0) ->
				{ Result = Result0 }
			;
...
peephole.match(UsedIds, single_token(identifier(Id)), Args, Result) -->
	peephole.search(Id, MaybeNewToken),
	{
...
peephole.match(_UsedIds, single_token(extra(popn(N1))), Args, Result) -->
	(
...
peephole.match(_UsedIds, single_token(operator(point)), Args, Result) -->
	{ top_three_real_args(Args, R, G, B, Rest) },
	{ Point = point(R, G, B) },
	{ Result = [single_token(extra(constant_point(Point))) | Rest ] }.

	% constant_point get functions
peephole.match(_UsedIds, single_token(operator(getx)), Args, Result) -->
	{ Args = [single_token(extra(constant_point(Point))) | Rest] },
	{ Point = point(X, _Y, _Z) },
	{ Result = [real_token(X) | Rest ] }.
peephole.match(_UsedIds, single_token(operator(gety)), Args, Result) -->
	{ Args = [single_token(extra(constant_point(Point))) | Rest] },
	{ Point = point(_X, Y, _Z) },
	{ Result = [real_token(Y) | Rest ] }.
peephole.match(_UsedIds, single_token(operator(getz)), Args, Result) -->
	{ Args = [single_token(extra(constant_point(Point))) | Rest] },
	{ Point = point(_X, _Y, Z) },
	{ Result = [real_token(Z) | Rest ] }.

	% Constant surface functions.
peephole.match(_UsedIds, single_token(operator(sphere)), Args, Result) -->
...
peephole.match(_UsedIds, single_token(operator(plane)), Args, Result) -->
	{ Args = [function(SurfaceFunc) | Rest] },
	{
		constant_surface_function(SurfaceFunc, SurfaceProperties)
	->
...
peephole.match(_UsedIds, single_token(operator(cone)), Args, Result) -->
	{ Args = [function(SurfaceFunc) | Rest] },
	{
		constant_surface_function(SurfaceFunc, SurfaceProperties)
	->
...
peephole.match(_UsedIds, single_token(operator(cube)), Args, Result) -->
	{ Args = [function(SurfaceFunc) | Rest] },
	{
		constant_surface_function(SurfaceFunc, SurfaceProperties)
	->
...
peephole.match(_UsedIds, single_token(operator(cylinder)), Args, Result) -->
	{ Args = [function(SurfaceFunc) | Rest] },
	{
		constant_surface_function(SurfaceFunc, SurfaceProperties)
	->
...
peephole.match(_UsedIds, single_token(operator(get)), Args, Result) -->
    { semidet_fail },
    { Args = [integer_token(I), array(TokenList) | Rest] },
    { X = list.det_index0(TokenList, I) },
    { Result = [X | Rest] }.

peephole.match(_UsedIds, single_token(operator(length)), Args, Result) -->
    { semidet_fail },
    { Args = [array(TokenList) | Rest] },
    { Result = [integer_token(list.length(TokenList)) | Rest] }.
