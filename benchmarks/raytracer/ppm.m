...
%-----------------------------------------------------------------------------%
%
% XXX I'm not sure if this interface is ideal; we might need to revisit it.
% -fjh.
%
...
:- pred ppm.draw_image_2(int::in, int::in, image::array_ui, io::di, io::uo)
    is det.
...
do_image_draw(Width, Height, X, Y, Image, !IO) :-
    ( Y < Height ->
        ( X < Width ->
            array.lookup(Image, (Y * Width) + X, Point),
