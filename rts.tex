...
\status{Prose is finished, I will proofcheck it myself before
I send it to Zoltan or Peter}
...
global queue.
An idle engine can run a spark by taking it from the beginning of the queue.
...
\Procedure{MR\_get\_global\_work}{}
  \State aquire\_lock($MR\_runqueue\_lock$)
  \Loop
    \If{$MR\_exit\_now$}
...
Newly created engines, expect for the first, also search for global work.
This is done by calling \getglobalwork.
\getglobalwork is shown in Algorithm \ref{alg:MR_get_global_work}.
Only one of the idle engines may execute \getglobalwork at a time.
...
if so this thread releases the lock then destroys itself.
If the system is not being shut down,
the engine will search for a runnable context.
If it finds a context it releases the runqueue lock, loads the context
...
It will attempt to get a context from the free list, if there is none it will
...
(which are set by the context that created the spar)
into the context.
If the engine could not find any work it will wait using the a condition
variable and the runqueue lock.
