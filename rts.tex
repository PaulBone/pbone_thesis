...
No. collections & 384 &        & 346  &          & 316  &          & 288  & \\
\hline
\hline
...
No. collections & 742 &        & 692  &          & 634 &           & 602 & \\
\end{tabular}
\end{center}
...
Using ThreadScope (Chapter \ref{chap:tscoe}),
analysed how much time these programs spend in collector and mutator time.
This results are shown in Table \ref{tab:gc_amdehl}.
The table shows the elapsed time, collector time, mutator time and the
number of collections for each program using 1--4 engines.
The times are averages taken from eight samples,
using a single garbage collector thread and an initial heap size of 16MB.
We compiled in support support for ThreadScope;
therefore these results differ slightly from those in table \ref{tab:gc}.
...
THey also made fewer collections if more Mercury engines were being used.
When using one Mercury engine,
the raytracer spends 16.9 seconds or 44.8\% of its time in the garbage
collector.
As we increase the number of Mercury engines, the time spent in the
collector decreases slightly, along with the number of collections.
However, the percentage of elapsed time spent in the collector increases,
because the elapsed time decreases.
The mutator time decrases almost linarly as the number of engines increases.
This shows how parallelism reducing the amount of mutator time,
but not having an affect on garbage collector time.
Futhermore, as predicted by Amdahl,
the non-parallelised part of the program, the garbage collector,
begins to dominate the program's execution.
mandelbrot\_highalloc spends less of its time in garbage collection than
raytracer.
This matches the results in \ref{tab:gc}:
mandelbrot\_highalloc benifits less from parallelism in the garbage
collector than raytracer because it spends less time doing garbage
collection.
Similarly, it benifits more from parallelism in Mercury than raytracer
because it spends more time executing the Mercury code.
In general,
mandelbrot\_highalloc has similar trends when compared with raytracer,
except that it spends more of its time in garbage collection as the number
of engines increases rather than less.
\paul{Why?}
...
\paul{XXX: Fix these results, they should be based on mutator time not
elapsed time}
