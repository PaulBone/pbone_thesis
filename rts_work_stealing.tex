...
The engine that is executing $C_{Orig}$ first suspendeds $C_{Orig}$ and then
...
thief left off,
this is gaurenteed because \var{MR\_victim\_counter} is protected by the
lock aquired on line 2.
...
\paul{XXX: Fix the context limit condition (allow it to suceed if the engine
...
context limits.
Specifing a limit of four contexts limits performance when using three or
