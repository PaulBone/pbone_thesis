...
\status{I am updating this after fixing some RTS bugs.
I now have to re-run the tests and then double check the prose.
After which I will continue with the next section.}
...
\footnote{
    ...
    \item[\code{bool MR\_pop\_spark(deque *d, spark *s)}]
    ...
    \item[\code{result MR\_steal\_spark(deque *d, spark *s)}]
...
declaration for the \code{top} and \code{bottom} fields of the deque
structure.
...
(lines 18--19).
The busy loop on lines 7--10 includes a \code{CPU\_busy\_loop\_hint}
instruction,
...
\Procedure{MR\_try\_steal\_spark}{$spark\_ptr$}
  ...
  \For{$attempt = 0$ to $max\_attempts - 1$}
