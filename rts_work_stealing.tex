...
used}
\begin{verbatim}
    void MR_join_and_continue(MR_SyncTerm *ST, MR_Code *ContLabel) {
...
10              MR_GOTO(ContLabel)
            } else {
                while (orig_context->MR_ctxt_resume != ContLabel) {
                    CPU_spin_loop_hint();
                }
...
25                   current_context->MR_ctxt_resume = ContLabel;
...
15          // Try to run a context
            ctxt = MR_get_runnable_context();
            if (ctxt != NULL) { 
                MR_release_lock(&MR_runqueue_lock);
                if (current_context != NULL) {
20                  MR_release_context(current_context);
                }
...
25              MR_GOTO(resume);
            ...
            // Try to run a spark.
            if ((current_context != NULL) ||
30                  (MR_num_outstanding_contexts < MR_max_contexts))
            {
                got_spark = MR_try_steal_spark(&spark);
                if (got_spark) {
                    MR_release_lock(&MR_runqueue_lock);
35                  if (current_context == NULL) {
                        ctxt = MR_get_free_context();
                        if (ctxt == NULL) {
                            ctxt = MR_create_context();
                        }
40                      MR_load_context(ctxt);
                        current_context = ctxt;
                    }
                    MR_parent_sp = spark->MR_spark_parent_sp;
                    current_context->MR_ctxt_thread_local_mutables =
45                      spark->MR_spark_thread_local_mutables;
                    MR_GOTO(spark->MR_spark_code);
                }
            }

50          MR_timed_wait(&MR_runqueue_cond, &MR_runqueue_lock);
        }
    }
\end{verbatim}
...
This check is on lines 29--30; it is true of either,
...
When sparks are placed on a contexts local stack the run queue's condition
...
This is done on line 50 using a call to \code{MR\_timed\_wait} with a
...
protected by \code{MR\_spark\_deques\_lock}.

\begin{algorithm}[tbp]
\begin{verbatim}
    void MR_try_steal_spark(MR_Spark *spark) {
        int         max_attempts;
        MR_Deque    *deque;
        MR_bool     got_spark;
 5
        MR_acquire_lock(&MR_spark_deques_lock);
        max_attempts = MR_MIN(MR_max_spark_deques, MR_worksteal_max_attempts);
        for (int attempt = 0; attempts < max_attempts; attempt++) {
            MR_victim_counter++;
10          deque = MR_spark_deques[MR_victim_counter % MR_max_spark_deques];
            if (deque != NULL) {
                got_spark = MR_steal_spark(deque, spark);
                if (got_spark) {
...
Line 7 sets the number of attempts to make, which is either the user
configurable \var{MR\_worksteal\_max\_attempts} or the size of the array,
whichever is smallest.
A loop (beginning on line 8) attempts to steal work until it succeeds or it
...
On line 12 we attempt to steal work from a deque,
...
Eventually \trystealspark may give up (lines 19--20).
