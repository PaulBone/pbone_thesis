...
(section \ref{sec:overlap_reccalls}),
and variable use time anlysis (section \ref{sec:backgnd_var_use_analysis}).
...
sites from the top-most level of \foldlthree's recursion,
only the value for the recursive call was calculated by the algorithms
in the previous section;
the other values are provided by the profiler.
This recursion level was chosen as so that the sum of the costs of calls in
the body of the procedure is approximatly equal to the cost of the procedure
from its parent call site.
The second and third columns in this table report static and deep coverage
data respectivly.
The deep profiling and coverage data was collected from an SCC of
...
which is a far cry from the true cost the call to this SCC (3,898,634csc).
...
feedback)
Therefore we have presented profiling results with optimisations both
