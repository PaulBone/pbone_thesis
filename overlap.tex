...
profiling data that is extreamly valulable if not necessary for declarative
langauges.
...
allows coverage to be infered for \emph{all} program points.
...
each time execution reaches the coverage point this slot is incremented.
This design was initially adequate, simple and would increase memory usage
(for the coverage points) by a constant factor.
...
\C{\textbf{Coverage}} & \Cbr{\textbf{Cost}} & \\ 
\hline
 7,851 & 3,358 & 2,689 & 3,898,634 & \code{foldl3(P, Xs0, !Acc1, ...) :-} \\
     - &     - &     - &         - & \code{~~~~(} \\
 2,142 &     0 &     0 &         0 & \code{~~~~~~~~Xs0 = []} \\
     - &     - &     - &         - & \code{~~~~;} \\
 5,709 &     0 & 2,688 &         0 & \code{~~~~~~~~Xs0 = [X $|$ Xs],} \\
 5,709 & 1,259 & 2,688 &     1,449 & \code{~~~~~~~~P(X, !Acc1, ...),} \\
 5,709 & 2,097 & 2,688 & 3,897,182 & \code{~~~~~~~~foldl3(P, Xs, !Acc1, ...)} \\
     - &     0 &     - &         - & \code{~~~~).} \\
\end{tabular}
\end{center}
\caption{Static and Deep coverage and cost data for \code{list.foldl3}}
...
Throughout the automatic parallelisation analysis
we use coverage and cost data together to calculate the costs of recursive
goals and variable production and consumption times 



Because we now traverse the call graph 
