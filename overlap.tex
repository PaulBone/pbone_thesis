...
\status{Draft finished, I do not know if I will read this myself before
asking for reviews or not}
...
the production time for \var{Acc1} in the recursive call (\code{map\_foldl}
is $1,625,051$.
...
There is another important piece of information that we compute:
...
\status{This is a draft version of this section, I wish to run the
benchmarks again to see if the strange trend disappears}
...
the raytracer program and a modified version of the mandelbrot program,
both from chapter \ref{chap:rts},
as well as a matrix multiplication program.
Matrixmult has abundant independent AND-parallelism.
Mandelbrot was modified, and now it uses dependent AND-parallelism
via the actual \code{map\_foldl} predicate from figure~\ref{fig:map_foldl}
...
\hline
indep    & 5.10 & 7.69 & 7.70 (0.66) & 3.87 (1.32) & 2.60 (1.96) & 1.97 (2.59) \\
naive    &    - &    - & 7.70 (0.66) & 3.87 (1.32) & 2.60 (1.96) & 1.97 (2.59) \\
overlap  &    - &    - & 7.70 (0.66) & 3.87 (1.32) & 2.60 (1.96) & 1.97 (2.59) \\
...
\hline
indep    & 15.3 & 15.3 & 15.2 (1.01) & 15.2 (1.01) & 15.2 (1.01) & 15.1 (1.01) \\
naive    &    - &    - & 15.2 (1.01) & 10.4 (1.48) &  5.9 (2.58) &  4.2 (3.67) \\
overlap  &    - &    - & 15.2 (1.01) &  9.9 (1.53) &  5.4 (2.82) &  4.0 (3.85) \\
...
\hline
indep    & 14.7 & 16.9 & 16.9 (0.87) & 16.9 (0.87) & 17.0 (0.87) & 17.0 (0.87) \\
naive    &    - &    - & 17.5 (0.84) & 13.7 (1.08) & 10.3 (1.44) &  8.9 (1.66) \\
overlap  &    - &    - & 17.4 (0.85) & 13.0 (1.13) & 10.4 (1.42) &  9.1 (1.63) \\
...
Table \ref{tab:autopar} presents our auto parallelisation results.
...
with speedups compared to the sequential version in parentheses.
...
Mercury uses four\footnote{
    We only use four real registers because these registers must
    be callee-save registers,
...
and first two general purpose registers.
The parallel version of the Mercury system
needs to use a real machine register
to point to thread-specific data (section \ref{sec:backgnd_merpar}),
...
All versions of matrixmult where parallelised the same way and performed the
same.
This is normal for such a small example with simple parallelism.
We discuss the much more complicated example of the Mercury compiler
below.
In an earlier version of our system,
the version that was current when we published this
research~\citep{bone:2011:overlap},
the naive parallelisation chose a to parallelise matrixmult's main loop
differently,
...
independent.
Therefore in \citet{bone:2011:overlap} we reported that naive performed
...
however the feedback given to the compiler is the same as overlap's,
there is no reason why it should perform slower as the generated code should
be the same.
\paul{XXX: Test them again, see if this reoccurs}
...
because it stores intermediate results such as colours and vectors in
records on the heap.
...
mode incorrect code.
Therefore the programs that use dependent parallelism,
raytracer and mandelbrot,
do not perform well when parallelised
(either automatically or manually).
Even if these programs do not reach the context limit,
the additional stacks created by the large number of contexts they create
need to be scanned for pointers by the garbage collector.
...
dependent parallel right recursive code does not scale to large numbers of
processors well.

We have verified this idea by
testing versions of the raytracer in which
each spawned-off goal computed the pixels for several rows, not just one,
and these versions yield speedups of about 3.3 on four engines.
This testing was done on a different machine to the above results so we
cannot compare the results directly.
...
We fix the right recursion problem in chapter \ref{chap:loop_control}.
...
in systems that support only independent AND-parallelism such as
