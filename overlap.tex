...
the first two are from chapter \ref{chap:rts},
they are the raytracer and a modified version of the mandelbrot program,
the third is a matrix multiplication program.
Mandelbrot's modifications make it use dependent AND-parallelism
via the \mapfoldl predicate in figure~\ref{fig:map_foldl}.
...
Raytracer achieves a speedups of 1.90 and 1.89 with four Mercury engines.
...
Any apparent difference is due to the high degree of variability in these
tests' results,
...
mode incorrect code.
Even if raytracer and mandelbrot do not reach the context limit,
...
and these versions yield better speedups.
\paul{Maybe re-test this so we can compare it}
...
the naive strategy chose to parallelise matrixmult's main loop
differently,
...
such as \citet{DBLP:journals/tcs/GrasH09}.
...
worthwhile \citep{harris_07_feedback_imp_par,lopez96:distance_granularity},
...
Mercury \citep{tannier:2007:parallel_mercury}.
...
\citep{Pontelli97automaticcompile-time}.
