...
usage.
The statis coverage data is tored in the \PS structures which are stored in
the program's static data, the \samp{.data}, \samp{.rodata} and \samp{.bss}
sections of the executuable;
in particular, the coverage point values themselves are in the \samp{.bss}
section.
Coverage profiling also creates a small increase the the size of the
executable program, as the program now contains some additional
instrumentation.
Likewise this instrumentation affects performance very slightly.
The effect is well within our goal of minimising coverage profiling's
overheads.
...
is acceptable.
Conversly we see that the amount of statically allocated memory decreases
...
This way each of the $i$ chunks repreresents the time before $q$ may consume the
...
acheived because of the work in chapter \ref{chap:loop_control}.
