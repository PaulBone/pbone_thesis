...
To do this the programmer can reprofile the program to generate fresh
...
The other source of inacuracy comes from misestimating the hardware's
performance on certain operations such as the cost of spawning off a new
task.
Such misestimations will have the same impact as variations in impact data.
...
\noindent
This is the approximate depth of the tree so we omit the ceiling operator.
...
\noindent
By induction,
the cost of a recursive call in a prefectly balanced divide and conquer
...
15\textsuperscript{th} lavel so $D$ must be 14.
...
The second cause of inacurate results comes from our assumption about
\partition's cost.
We assumed that partition always has the same cost at every level of
the call graph.
However \partition's cost is directly proportal to the size of its input
list which is smaller at deeper levels.
...
total\_part\_cost(D)   ~&= part\_cost(length(D)) +
    \begin{cases}
        2total\_part\_cost(D - 1) & \text{if}~ D > 0 \\
        0                         & \text{if}~ D = 0 \\
    \end{cases} \\
...
\paul{This seems to approch 6 as D approches $\inf$.
If I knew how to remove the recursive definition above I could prove this
and show that in a big enough tree inaccuracies don't matter.
...
%Consider the call graph in figure \ref{fig:quicksort_acc_callgraph},
%at the lowest level the base cases are executed and partition is not called.
%At the next level up, the length of each input list is one, the pivot value,
%and the rest of the list is empty, the cost of \partition is 1csc
%(the cost of calling \partition).
%At the next level (the second level from the top) the lists have three
%items including the pivot, \partition is executed on a list of two items and
%its cost is 3csc.
%Finally at the top most level, the list has seven items, and \partition is
%called on a list of six items, its cost is 7csc.
%There are seven calls to \partition the and average cost is
%$(7 + 2\times3 + 4\times1) / 7 = 17/7 \approx 2.43\text{csc}$
%The calls to \partition below the second level from the top are the ones
%that should be used in our calculation of the recursive calls' costs,
%these are the calls at the second lowest level which cost 1csc.
