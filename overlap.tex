...
\hline
no profiling     & -    & - &    - & 1.01s & - &    - \\
no coverage      & 6.18s & - & 20MB & 3.79s & - & 14MB \\
static coverage  & 6.32s & - & 21MB & 3.92s & - & 15MB \\
dynamic coverage & 6.60s & - & 24MB & 4.02s & - & 18MB \\ 
\end{tabular}
\end{center}
...
Table \ref{tab:coverage_prof_overheads} shows the overheads of profiling a
Mercury program, including coverage profiling.
...
The two groups of result columns give results using different optimisations.
Certain optimisations, such as inlining,
transform a program in ways that would make
it hard for a programmer to use the profiler.
Therefore we normally disable these optimisations during compilation.
However automatic parallelisation is normally used with other optimisations,
and therefore we should enable these optimisations when compiling the
profiling version of a program that will be used for automatic
parallelisation.
We have shown results with these optimisations disabled, and with them
enabled.
