...
\paul{Write about how the static coverage information is not stored on the
heap, maybe measure .data and .bss}
...
it keeps track of when those chunks can execute.
...
Figure~\ref{fig:overlap2-swap} shows the overlap of a parallel conjunction with
two shared variables \var{A} and \var{B}.
...
If $p$ has produced the value of ${Var}_i$ by the time $q$ needs it,
there $q$ does not need to wait for it;
the variable \var{B} in the figure shows this case.
However, it is also possible that $p$ will produce ${Var}_i$
only after the time at which $q$ would like to use it.
In that case, $q$ will suspend the variable's future,
and will resume only when $p$ signals that it is available;
we can see this for \var{A} in the the figure.
The algorithm determines which alternative is correct on line 9,
$ParWantTime_{q, i}$ is the that $q$ wants to consume the
$i$\textsuperscript{the} variable,
and $ProdTimeMap_{p}[Var_i]$ is the time at which $p$ produces the variable,
this is a map as variables may be consumed in a different order than they
where produced.
