...
\item \paul{Minor changes to variable use algorithm.}
\item \paul{Coverage (and variable use) now work on deep profiling data.}
\end{itemize}
...
\status{I have finished this section, but I will read it once more before I
...
profiling data that is extremely valuable if not necessary for declarative
languages.
We want to make the best use of this data when auto-parallelising programs.
This means that our algorithms should compute results specific to ancestor
call chains;
...
These two analyses use the profiler's coverage profiling feature (section
...
While the profiler provides deep cost data,
it provides only \emph{static} coverage data,
data not associated with any specific \PD structure and therefore not
specific to an ancestor call chain.

\plan{Explain the shallow coverage information.}
By comparison, the instrumentation introduced by coverage profiling stores
its data in the \PS structures.
This data is represented by using two arrays and an integer describing the
arrays' lengths.
...
At the time when coverage profiling was implemented static coverage data was
adequate.
Now that we are using deep cost data throughout the auto parallelisation
tool this can cause problems.
Figure \ref{fig:static_and_deep_coverage} shows \foldlthree from
Mercury's standard library,
annotated with data collected from a profiling run of the Mercury compiler.
...
cost for the procedure as a whole on line 1,
and the average per-call cost for the recursive call to P on line 6.
The average cost of the recursive call (line 7) is calculated using the
...
We can conclude that across the whole program,
on average \foldlthree does not make very deep recursions
(it is called on short lists) and the cost of the higher order call is
cheap.

When we consider the deep profiling information specific to a particular
ancestor call chain,
...
recursive calls.
This recursion is rather deep having a maximum depth of 2,689.
Here is where deep coverage information becomes necessary,
...
above.
Furthermore, the coverage data is necessary to accurately compute the cost of
the recursive call when called from the second level of recursion.
In this case the cost is 3,897,182;
but had we computed this using the erroneous depth the result would be
4,464.
...
Each \PD structure links the procedure to its uses in the program's call
graph (with the exception of recursive calls).
Therefore we can move the array containing the coverage points' execution
counts into the \PD structure.
Because there are more \PD structures than \PS structures in a program's
profile this will make the profile larger, consuming more memory.
It is important to avoid affecting the performance impact of profiling,
for both the user's experience and the risk of distorting the program's
profile.
...
Although the non-profiling version of the compiler was not compiled with
...
the \emph{heap growth}, the amount by which the program break
(see the \code{sbrk(2)} unix system call) moved during the execution;
and the size of the resulting profiling file.

We tested with different optimisations because
certain optimisations, such as inlining,
transform a program in ways that would make it hard for a programmer to
recognize their program's profile.
Therefore we normally disable these optimisations when compiling with
profiling support.
...
the program.
Therefore when compiling profiling versions of programs for the purpose of
automatic parallelisation (or other types of profiler feedback) we enable
optimisations that would normally confuse a programmer using the profiler.
The result columns with optimisations disabled are the results that one
should expect when profiling a program so that a developer can use the
profiler.
Conversely, the results with optimisations enabled are the results that one
should expect when profiling a program for automatic parallelism or some
other feedback-driven optimisation.

We can see that enabling profiling slows the program down by at least a
factor of three,
or a factor of five if optimisations are disabled.
While this is a considerable slow down we would like to remind the reader
that the profile cannot be distorted by a slowdown as we use call sequence
counts as our unit of time.
The difference between with and without optimisations is unsurprising.
It is well established that simple optimisations such as inlining can make a
significant difference to performance.
We can also see that optimisations affect how much memory the program uses
and how large the resulting profile data file is.
...
Likewise, this will create a program with less instrumentation and therefore
...
is acceptable.
Deep coverage information does have a slight performance impact over static
...
As stated earlier,
our goal is to recognize how dependencies affect the amount of parallelism
...
\begin{algorithm}
\begin{algorithmic}
\State $CurSeqTime_q \gets 0$
\State $CurParTime_q \gets 0$
\State sort $ConsList_q$ on $ConsTime_{q, i}$
\For{$(Var_i, ConsTime_qi) \in ConsList_q$}
    \State $Duration_{q, i} \gets ConsTime_{q, i} - CurSeqTime_q$
    \State $CurSeqTime_q \gets CurSeqTime_q + Duration_{q, i}$
    \State $ParWantTime_{q, i} \gets CurParTime_q + Duration_{q, i}$
    \State $CurParTime_q \gets$ max($ParWantTime_q, Prod_{p, i}$)
\EndFor
\State $DurationRest_q \gets SeqTime_q - CurSeqTime_q$
\State $SeqTime_q \gets CurSeqTime_q + DurationRest_q$
\State $ParTime_q \gets CurParTime_q + DurationRest_q$
\end{algorithmic}
