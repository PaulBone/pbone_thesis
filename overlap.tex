...
considering only those whose cost was above its own threashold.
...
that have just two types execution paths through them:
...
We call this the average depest recursion:
...
problematic.
We know that if a good value for \code{Pivot} is choosen \quicksortacc runs
optimally,
dividing the list in half with each recursion.
Therefore there are a minimum of
$\log_2 (Calls_{RecCallSites} / Calls_{ParentCallSites})$ recursions.
However,
...
\begin{equation*}
Depth = \log_2 (Calls_{RecCallSites} / Calls_{ParentCallSite})
\end{equation*}



\mapfoldl is one of the simplist procedures that fits this simple recursion
pattern.
We are able to handle procedures with any number of base cases, and any number
of recursive cases provided that the recursive cases contain eactly one
recursive call.
An algorithm 
