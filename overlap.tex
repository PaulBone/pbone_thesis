...
We start with traversing the call graph in section \ref{sec:overlap_dfs}.
Then in section \ref{sec:overlap_reccalls}
we describe how to calculate information about recursive calls missing from
the profiling data.
...
The previous version of the parallelisation feedback tool analysed the the
procedures that contributed the most to the program's execution time.
When considering each procedure it used profiling data gathered from every use
of that procedure.
Our new version makes a depth-first search over the call tree,
which was described as a tree of SCCs (strongly connected components)
in section \ref{sec:backgnd_deep}.
The same procedure can belong to multiple SCCs depending on its ancestor
calls and call sites.
Each SCC has one entry procedure and one or more other procedures.
The other procedures are mutually recursive with the entry procedure,
either directly or indirectly,
and thus part of the SCC.
The profiling data for the procedures in each SCC is specific to the uses of
the procedures when called from the SCC's ancestors.
Therefore,
the depth first search can visit the same procedure more than once,
each time with different call cost data.
This can cause the analyses described below to generate different answers
regarding the parallelisation of any procedure.


This means that as we analyse each procedure
(SCCs are made up of one or more procedures)
that the data to analyse each procedure is gathered from the calls made to
that procedure when it is called from that \emph{ancestor context}.
An ancestor context is the chain of call sites and callees that lead to the
execution of a SCC.

This is explained in more detail in section \ref{sec:overlap_dfs}.

The previous version of the parallelisation feedback tool analysed the the
procedures that contributed the most to the program's execution time.
When considering each procedure it used profiling data gathered from every use
of that procedure.
We limited the number of procedures examined by only examining those whose
total per-call cost was above a particular threshold.


However, unlike the previous version of the feedback tool
...
\paul{Describe the costs of recursive calls}
