...
\noindent
By induction,
the cost of a recursive call site at depth $D$ (0 being the deepest)
is:
...
Then its typical cost is $10 + 4.5(10,000 + 1) = 45,014.5$ call sequence counts.
...
There are 15 notes in ``qs'''s call tree,
...
If consistently worst-case pivots are chosen then \quicksortacc call graph
will resemble a stick.
Pathologically bad cases are rare and are considered performance bugs;
programmers will usually want to remove such bugs in order to improve
sequential execution performance before they attempt to parallelise their
program.
...
is not quite representative of the optimised program's future.
In section \ref{sec:overlap_approach} we explained that these variations are
harmless.
